...
describe MockConstants do
  subject{MockConstants::Base.new Kernel}
  ...
  describe "API" do
    ...
    it "should should default to targeting Object" do
      MockConstants::Base.new.target.must_equal Object
    end
  ...
  describe "API shortcuts" do
    it "MockConstants::Base.on(...) is a shortcut for MockConstants::Base.new.on(...)" do
      subject.expects(:on)
      MockConstants::Base.expects(:new).returns(subject)
      MockConstants::Base.on(Kernel)
    end
    
    it "MockConstants::Base.with(...) is a shortcut for MockConstants::Base.new.with(...)" do
      subject.expects(:with)
      MockConstants::Base.expects(:new).returns(subject)
      MockConstants::Base.with({},[]){}
    end
    
    it "MockConstants::Base.new.remove(:A) is a shortcut for MockConstants::Base.new.remove([:A])" do
